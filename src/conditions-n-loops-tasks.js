...
function isIsoscelesTriangle(a, b, c) {
  const sides = a === b || a === c || b === c;
  const sum = a + b > c && a + c > b && b + c > a;
  return sides && sum;
}
 ...
 * In this task, the use of methods of the String and Array classes is not allowed.
 *
 * @param {number} currentNumber - The number to convert.
...
function isPalindrome(str) {
  ...
  for (let i = 0; i < str.length; i += 1) {
    reverceWord = str[i] + reverceWord;
  }
  for (let i = 0; i < str.length; i += 1) {
    if (str[i] !== reverceWord[i]) return false;
  }
  return true;
}
